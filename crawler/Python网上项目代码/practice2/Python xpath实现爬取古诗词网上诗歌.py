...
DATA=[]
def getHIMEtext(url,headers,timeout=10):
    try:
...
def xpathParser(text):
        ...
        name_list=htmlElement.xpath()
        dynasty_list=htmlElement.xpath()
        authot_list=htmlElement.xpath()
        content_list=[]
        poetries=htmlElement.xpath()
        # print(etree.tostring(poetries[0],encoding = 'utf-8').decode('utf-8'))
        for poetry in poetries:
            row_content=''.join(poetry.xpath('.//text()'))#这里，可千万不能掉，否则会忽略poetry
            content_list.append(row_content.replace('\n',''))
        row_likes_list=htmlElement.xpath('')
        like_list=[int(like.strip()) for like in row_likes_list]
        for value in zip(name_list,dynasty_list,authot_list,content_list,like_list):
...
if __name__ == '__main__':
    ...
    num=input('请输入爬取的页数(1-100):')
    for i in range(eval(num)):
        url=row_url.format(i+1)
        text=getHIMEtext(url,headers)
        if text=='':
